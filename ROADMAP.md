...
| Trợ lý thực thi công việc cho kỹ sư (009) | `PARTIAL`: US1 và US2 cùng nền an toàn thực thi và chính sách phân quyền đường dẫn đã kiểm chứng đạt 100% (Báo cáo lỗi xưởng, Rà soát thiết kế công đoạn). US3 (Sửa mã) tạm hoãn chờ runtime OpenCode thực tế chứng minh đầy đủ; US4 (Hàng đợi công việc bền vững) đang chuẩn hóa UI i18n không dùng từ kỹ thuật và nối trực tiếp luồng enqueue/process thực tế. |
...
| Tổng hợp đa nguồn (002) | `IMPLEMENTED_PENDING_CURRENT_FULL_SUITE` — lát chất lượng 2026-09-12 trên BGE-M3 Hybrid: planner cục bộ tách câu nhiều vế, truy xuất không cắt 3 nguồn khi đa ý, tổng hợp `Ý 1`/`Ý 2`/`Còn thiếu`. Test tập trung lát này đạt. Câu một vế (kiểu BQ02) vẫn có thể bị prefilter 3 khi chuẩn bị. Full pytest vẫn 2778 passed / 2 failed ngoài 002. Không tuyên bố ngang NotebookLM. |
| Recall hybrid lệch chữ (012) | Đang triển khai — truy xuất mọi nguồn ready khi câu Latin vs kho CJK; hybrid ưu tiên dense; retry khi chỉ ra 1 tài liệu. Không hardcode BQ/tên file. Không tuyên bố ngang NotebookLM. |
| Quản lý cuộc trò chuyện (004) | `TECHNICAL_PASS` — 2026-09-13: `pytest -q -m "not desktop_packaging"` **2774 passed, 29 deselected, 0 failed** (182s). Xóa phiên có xác nhận tên hội thoại, không màn hình trắng; đổi tên/chọn phiên rõ. Test đóng gói desktop (`verified_wheels`) tách marker `desktop_packaging`, không thuộc cổng 004. Không tuyên bố nghiệm thu xưởng. |
...
| Thanh nhập chat hiện đại (007) | `TECHNICAL_PASS` — Playwright smoke **12/12 PASS** ngày 2026-09-23 (`scripts/smoke_007_modern_chat_composer.py`, `local_runs/smoke_007_t037/result.json`). Sáu kịch bản S7–S11b mới kiểm bằng DOM/kiểu tính toán: mặt sáng ba vùng và tên điều hướng tiếng Việt, bong bóng hỏi đáp và bề rộng dòng, canvas đồ thị bằng chứng (sidebar không phủ canvas, đủ nhóm thực thể, có bảng kiểm tra), nút đóng đồ thị, giữ câu hỏi chờ nguồn không phần trăm giả, ba bước chờ theo đúng trạng thái thật. Tập trung `135 passed`, `compileall` sạch, `cli audit` `PASS`. Vòng trích dẫn cần nhà cung cấp câu trả lời nên smoke gieo sẵn dữ liệu hội thoại/dấu vết trong thư mục tạm. Không tuyên bố nghiệm thu xưởng. |
...
| Nhận log JIG Iris, ngưỡng thật, mail cảnh báo (016) | `DONE` (phạm vi kỹ thuật) — bộ chuyển đổi log Iris thật (572–1.059 cột) tách ma trận rộng thành bản ghi chuẩn, bỏ canh lỗi có ghi lý do (ba biến thể `999`/`999.9`/`9999.9`), giữ `RESULT:<MÀU>` làm nhãn; Thẻ 1 nhận thẳng tệp log hoặc ba tệp chuẩn; ngưỡng trên/dưới thật đọc từ tệp Spec/CamPos **theo đúng số sê-ri và thời điểm đo**; lệnh ngưỡng qua chat; thẻ đề xuất gửi mail có mã duyệt dùng lại ảnh đang xem trước. Bằng chứng: `tests/test_iris_log_intake.py` **48 passed**, `tests/test_smtp_config_mail_ui.py` **11 passed**, bộ liên quan **143 passed**, `pytest -q -m "not desktop_packaging"` **3043 passed / 9 failed** (9 lỗi có sẵn ngoài phạm vi), `compileall` sạch, `cli audit` `PASS`, `check_docs.py` `PASS`, `import workspace_chat_app` thoát 0. **Kiểm toán độc lập 4 vòng: 15 lỗi thật đã sửa hết, vòng cuối ký xác nhận A–E với `findings` rỗng.** Chưa gửi mail thật (chưa có máy chủ SMTP nội bộ) nên hạng mục 3 dừng ở thẻ đề xuất theo đúng ghi chú đặc tả. |
