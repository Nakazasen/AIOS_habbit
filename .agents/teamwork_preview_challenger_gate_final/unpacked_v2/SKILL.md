---
name: excaliflow
description: "Tự động phân tích codebase bất kỳ hoặc đồ thị tri thức (Graphify / Understand), tạo sơ đồ kiến trúc Mermaid theo phong cách vẽ tay Excalidraw và xuất ra giao diện web tương tác độc lập (Pan/Zoom v4.5.1, Collapsible Sidebar Ctrl+B, Live Editor, Export PNG/SVG)."
---

# Excali-Flow Skill: Visual Architecture & Diagramming Protocol (v2)
...
python "C:\Users\Admin\.gemini\config\skills\excaliflow\scripts\generate_diagram.py" --open

# 2. Quét một dự án ở thư mục bất kỳ và chỉ định file đầu ra:
python "C:\Users\Admin\.gemini\config\skills\excaliflow\scripts\generate_diagram.py" --dir "D:\MyProject" --out "D:\MyProject\architecture_viewer.html" --open

# 3. Bật chế độ Watch Mode (tự động cập nhật sơ đồ mỗi khi lưu file Ctrl+S):
python "C:\Users\Admin\.gemini\config\skills\excaliflow\scripts\generate_diagram.py" --watch

# 4. Tự động cài đặt Git Pre-commit / Post-commit Hook vào dự án:
python "C:\Users\Admin\.gemini\config\skills\excaliflow\scripts\generate_diagram.py" --install-hook
```
