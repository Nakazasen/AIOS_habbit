...
### Giai đoạn 4: Thẩm Mỹ Nâng Cao & Tương Tác Flowsint 2.0 (Đã hoàn thành)

- Tích hợp thanh công cụ nổi Floating Canvas Controls (Zoom In/Out, Fit to View, Fullscreen).
- Thay thế cạnh nối bằng đường cong Cubic Bezier với hiệu ứng xung điện chuyển động (`stroke-dasharray animated pulse`).
- Nâng cấp Collapsible Inspector Panel với thước đo Confidence Gauge Bar và cây liên kết hai chiều.
- Tích hợp bộ lọc thực thể thời gian thực và tìm kiếm tương tác.

### Giai đoạn 5: Bước Nhảy Vọt - Mạng Thiên Hà Động Lực Học & Mini Radar Inspector (Đang thực hiện)

- **Động cơ Mô phỏng Động lực học Tự nhiên (In-Process Force-Directed Simulation)**:
  - Triển khai thuật toán vật lý thuần Vanilla JS (Coulomb repulsion, Hooke springs, Center gravity).
  - Tự động bố trí các node theo hình chùm sao thiên hà (Galaxy Layout), phân cụm xung quanh Hub Node (Question/Answer).
  - Hỗ trợ kéo thả node (Drag-and-drop physics) với hiệu ứng đàn hồi tự nhiên ở 60fps.
  - Phân cấp hiển thị Level of Detail (LOD): Các node trích dẫn/chunk hiển thị dạng hạt sáng siêu nhỏ (`micro-dot`), các node trung tâm hoặc được chọn hiển thị dạng viên thuốc (pill capsule) phát sáng neon.
- **Bộ Định Vị Radar Tiểu Vũ Trụ (Mini Radar / Ego Graph)**:
  - Xây dựng thành phần SVG Mini Radar trong cột Inspector Panel.
  - Tọa độ hóa node đang chọn ở tâm `(0, 0)` và các node liên kết lân cận nằm trên vòng tròn quỹ đạo bán kính $R$.
  - Hiển thị số lượng láng giềng: `{count} liên kết kế cận` hệt Flowsint nguyên bản.
  - Tương tác hai chiều: Nhấp vào node trên Radar lập tức chuyển tiêu điểm trên Canvas chính.
- **Kiểm thử & Đảm bảo Cổng Chất Lượng**:
  - Viết unit test cho Force-directed simulation logic, Micro-dots và Mini Radar DOM.
  - Vượt qua 100% Quality Gates (`compileall`, `pytest`, `cli audit`, runtime import).
