...
```json
{
  ...
  "repo": {
    ...
    "repo_path_policy": "local_owner_only_absolute | logical_relative",
  ...
  "privacy": {
    ...
    "destination": "local_owner_only | external_manual_agent | owner_managed_chat",
...
- **Bản chất của SHA-256:** SHA-256 đóng vai trò là một integrity hash/checksum để kiểm tra tính toàn vẹn của tệp tin, tuyệt đối không phải là một chữ ký số (digital signature) có tính xác thực nguồn gốc. SHA-256 only provides integrity checking in A17. It is not a digital signature and does not prove author identity. Any authenticity/signing scheme is out of scope for A17 MVP.
- **Thuật toán băm toàn vẹn:** Sử dụng **SHA-256** làm hàm băm duy nhất. **Tuyệt đối không dùng MD5** làm integrity hash cho A17.
- **Wording Destination:** Trường `destination` trong task pack (ví dụ: `local_owner_only`, `external_manual_agent`, `owner_managed_chat`) chỉ đóng vai trò là chính sách bảo mật / phân phối (policy metadata) phục vụ cho owner-managed workflow, tuyệt đối không phải là một đối tượng thực thi (execution target). Bridge không tự động gọi router hay provider thật.
...
Absolute repository paths are only allowed in local_owner_only task packs that remain on the owner’s machine. They are forbidden in external_manual_agent, owner_managed_chat, cloud, or any out-of-process pack. External packs must use logical_repo_id and relative paths only.

Chính sách cụ thể cho từng loại đích (destination):

| destination | absolute_repo_path | allowed path style |
| --- | --- | --- |
| `local_owner_only` | allowed if needed | absolute + relative |
| `external_manual_agent` | forbidden | logical_repo_id + relative only |
| `owner_managed_chat` | forbidden | logical_repo_id + relative only |
...
- Các đường dẫn đặc thù của máy (machine-specific path)
- Đường dẫn mạng UNC (UNC path)
- Đường dẫn đến file cấu hình nhạy cảm/secrets/runtime (`.ai/`, `local_cases/`, `task.md`, v.v.)
...
10. **Tấn công chèn lệnh (command injection / shell interpolation):** Gài mã độc vào trường `declared_commands` trong report để lừa importer thực thi lệnh độc hại khi parse.
