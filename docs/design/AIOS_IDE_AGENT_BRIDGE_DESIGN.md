...
```json
{
  ...
  "privacy": {
    ...
    "destination": "MockRouterAdapter | CloudProvider",
...
13. **Accidental commit/push / report replay:** Gửi lại report cũ để ghi đè case state hiện tại.
...
2. **AI-GW-A17A:** Task Pack Export MVP (triển khai module tạo và xuất file task pack có ký số SHA-256).
...
> [!IMPORTANT]
> - `AIOS-GW-A17-DESIGN` là gate thiết kế tài liệu duy nhất. Các subgate tiếp theo chưa được mở và hoạt động implementation chưa bắt đầu.
