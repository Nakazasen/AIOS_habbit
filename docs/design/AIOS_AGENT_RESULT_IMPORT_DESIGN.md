...
- **Thông tin bắt buộc trong Observed Evidence:** Để đảm bảo tính toàn vẹn, mỗi observed evidence được ghi nhận phải chứa đầy đủ các trường thông tin tối thiểu sau:
  - Tên và phiên bản của verifier (`verifier_name`, `verifier_version`);
  - Thời gian quan sát thực tế (`observation_time`);
  - Thông tin về nhánh và head commit hiện tại của repository (`repo_branch`, `repo_head`);
  - ID lệnh hoặc ID bài test cố định được cấu hình trước (`fixed_command_id` / `fixed_test_id` nếu có);
  - Mã thoát và kết quả thực thi (`exit_code`, `result`);
  - Bản tóm tắt kết quả an toàn (`safe_summary`).
  *Lưu ý quan trọng:* Bộ observer tuyệt đối không được đọc và thực thi các câu lệnh tùy ý lấy từ trường `declared_commands` của report mà chỉ chạy các lệnh kiểm chứng cố định được định nghĩa trước bởi owner.
