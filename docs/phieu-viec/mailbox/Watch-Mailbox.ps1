# Watch-Mailbox.ps1 (v3) — watcher + giám sát OMP, vòng lặp cưỡng chế 2 đầu
#
...
#   - `xong-cho-duyet` -> popup (Muse sẽ review)
...
# Cách tìm: mở OMP lên, vào Task Manager > Details, xem tên cột Name.
$ompProcessName = "omp"
...
$AUTO_LAUNCH = $false
# Lệnh mở OMP + tham số prompt lần đầu. Ví dụ (điền đúng tool của bạn):
...
while ($true) {
    try {
        ...
        if ($status -eq "moi") {
            ...
            if (-not $ompRunning) {
                # OMP đang rảnh + có ticket -> lùa nó làm
                ...
                # OMP đang mở nhưng chưa nhận ticket -> nhắc nhẹ 1 lần
        ...
        elseif ($status -eq "dang-lam") {
            if (-not $ompRunning) {
                ...
                # Đang làm thì thôi - im lặng, chỉ canh kẹt tiến độ
        ...
        @{
            ...
            launchedTicket = $launchedTicket; updated = $now.ToString("s")
