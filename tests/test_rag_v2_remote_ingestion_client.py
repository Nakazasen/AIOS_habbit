...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        self.send_response(200); self.send_header("Content-Type","application/json"); self.end_headers(); self.wfile.write(b'{"job_id":"job-1","status":"QUEUED"}')
    def do_GET(self):
        if self.path.endswith("/bundle"):
            start=int(self.headers.get("Range","bytes=0-").split("=")[1].split("-")[0]); body=self.download[start:]; self.send_response(206); self.end_headers(); self.wfile.write(body)
        else:
            self.send_response(200); self.send_header("Content-Type","application/json"); self.end_headers(); self.wfile.write(b'{"job_id":"job-1","status":"READY"}')
    def do_PATCH(self):
        ...
        self.send_response(204); self.send_header("Upload-Offset",str(len(self.uploaded))); self.end_headers()
