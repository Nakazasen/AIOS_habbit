...
7. **Nền LSU Iris & Dự đoán lỗi (Mốc 0–5 `PASS`, Mốc 6 US12 code xong T058–T065, còn T066 kiểm toán):** Nạp log JIG qua Omnibar, gửi email kèm biểu đồ SPC Monozukuri chuẩn Nhật, mở cổng API Realtime nhận streaming log JIG, phân cấp Zero-UI bảo vệ luồng chat cá nhân (theo `specs/008-evidence-case-loop`).
8. **Ghi nhận kiểm chứng US12 ngày 2026-09-18 (để demo tại công ty):** Đã đối chiếu sheet `các bước` Bước 1 trong `AI_LSU_du_doan_loi.xlsx` và 7 slide `AI cảnh báo lỗi LSU.pptx` với code. Dán từng dòng log qua Omnibar (T058), thẻ kiểm tra tức thì đối chiếu USL/LSL + EWMA (T059), biểu đồ SPC Monozukuri PNG 300 DPI + SVG (T060), mail cảnh báo có duyệt trước khi gửi (T061), bảng cấu hình texte đổi bằng câu chat tự nhiên (T062), cổng API `POST /api/v1/jig/stream-log` nghe tĩnh lặng chỉ bật thẻ khi drift/chạm ngưỡng (T063–T064), cô lập phiên trực ban/cá nhân (T065) — đều đã có code và 38 test đạt. Kiểm chứng nhanh đạt: `compileall`, `cli audit` (`PASS`), `import workspace_chat_app`. Toàn repo còn 1 test fail chưa xác định nên chưa tuyên bố `PASS` toàn đợt và chưa đóng T066. Còn thiếu: chọn biểu đồ trong giao diện khi nhập file CSV (hạn 15/10/2026) và hạ tầng server JIG đẩy về AI (việc nhà máy). Mốc thời gian: 18/09/2026 chốt kiểm chứng kỹ thuật và mở T066; 23/09/2026 demo dán log + ngưỡng đúng hạn Excel; 15/10/2026 bù phần chọn biểu đồ và kiểm tra Bước 1; 11/2026 dùng thử thu thập cải tiến; 12/2026 chạy thử theo dõi ổn định; 01/2027 chạy thật khi có server JIG, SMTP thật và dữ liệu thật. Test nhanh: `uv run --no-sync --group dev pytest -q tests/test_omnibar_jig_log_ingest.py tests/test_spc_chart.py tests/test_alert_mailer.py tests/test_alert_config_chat.py tests/test_stream_api.py tests/test_jig_chat_wire.py tests/test_session_isolation.py`; chỉ dùng fixture tại `tests/fixtures/lsu_iris/`, dữ liệu thật giữ nhãn `local_only`, không đưa vào Git hay prompt cloud.
