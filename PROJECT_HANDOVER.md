...
- **Feature 009 — Trợ lý thực thi công việc cho kỹ sư:** `TECHNICAL_PASS` ngày 2026-09-13. Đã hoàn tất 100% toàn bộ 7 giai đoạn (T000–T030). US1/US2 tạo báo cáo lỗi có nguồn/biểu đồ và bản nháp rà soát thiết kế từ tệp cục bộ đã chọn; US3 sửa mã trong worktree cách ly (`tempfile.mkdtemp`), chạy test thật, diff thu gọn và hỗ trợ đưa vào workspace hoặc hoàn tác; US4 hàng đợi bền vững trong SQLite (`agent_work_items`), khóa độc quyền `WorkspaceWriterLock` theo workspace, FSM phục hồi restart và hiển thị trạng thái tiếng Việt trên UI; Giai đoạn 7 vượt qua toàn bộ kiểm thử bảo mật (path traversal, secret isolation, Unicode, prompt injection, Windows paths, Antigravity bridge non-regression), cập nhật Threat Model (TM-15 đến TM-18) và Privacy Impact Assessment. `antigravity_bridge.py` tiếp tục là nguồn AI cho Workspace Chat và giữ nguyên vẹn.
